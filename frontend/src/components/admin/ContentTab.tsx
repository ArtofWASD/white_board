...
export const ContentTab: React.FC = () => {
  ...
  const [activeContentTab, setActiveContentTab] = useState<'wods' | 'exercises'>('wods');
  const [wods, setWods] = useState<any[]>([]);
  const [globalExercises, setGlobalExercises] = useState<any[]>([]);
  const [loadingContent, setLoadingContent] = useState(false);
  
  const [isContentModalOpen, setIsContentModalOpen] = useState(false);
  const [contentModalType, setContentModalType] = useState<'wod' | 'exercise'>('wod');
  const [editingItem, setEditingItem] = useState<any>(null); // If null, it's create mode
  const [newWod, setNewWod] = useState({ name: '', description: '', type: 'GIRL', isGlobal: true });
  ...
  const fetchContent = async () => {
      ...
      try {
          const [wodsRes, exercisesRes] = await Promise.all([
      ...
      if (contentModalType === 'wod') {
          try {
              ...
              if (res.ok) {
                  ...
                  setNewWod({ name: '', description: '', type: 'GIRL', isGlobal: true });
              } else {
  ...
  const handleDeleteContent = async (item: any, type: 'wod' | 'exercise') => {
      if (!confirm('Вы уверены, что хотите удалить этот элемент?')) return;
      try {
          const url = type === 'wod' ? `/api/admin/wods/${item.id}` : `/api/admin/exercises/${item.id}`;
  ...
  const openEditModal = (item: any, type: 'wod' | 'exercise') => {
      setEditingItem(item);
    ...
    <div className="bg-white shadow-md rounded-lg overflow-hidden flex flex-col h-full">
        ...
        <div className="p-4 flex justify-end">
            <button 
                onClick={() => {
                    setEditingItem(null);
                    setContentModalType(activeContentTab === 'wods' ? 'wod' : 'exercise');
                    if (activeContentTab === 'wods') setNewWod({ name: '', description: '', type: 'GIRL', isGlobal: true });
            ...
            >
                ...
                Добавить {activeContentTab === 'wods' ? 'WOD' : 'Упражнение'}
            </button>
        </div>
        ...
        <Modal
              ...
              title={editingItem ? (contentModalType === 'wod' ? 'Редактировать WOD' : 'Редактировать Упражнение') : (contentModalType === 'wod' ? 'Создать WOD' : 'Создать Упражнение')}
              size="md"
              footer={
              ...
              {contentModalType === 'wod' ? (
                  <div className="space-y-4">
                      ...
                      <div>
                          ...
                          >
                              <option value="GIRL">GIRL (Девушки)</option>
