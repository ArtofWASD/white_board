...
export const OrganizationsTab: React.FC = () => {
  ...
  const handleToggleOrganizationBlock = async (org: any) => {
    if (
      !confirm(
        `Вы уверены, что хотите ${org.isBlocked ? "разблокировать" : "заблокировать"} организацию ${org.name}?`,
      )
    )
      return
    ...
    setOrganizations(
      organizations.map((o) => (o.id === org.id ? { ...o, isBlocked: newStatus } : o)),
    )
        ...
        <>
          ...
          <div className="md:hidden space-y-4 p-4">
            {organizations.map((org) => (
              <div
                key={org.id}
                className={`bg-white rounded-lg shadow p-4 ${org.isBlocked ? "opacity-75" : ""} border border-gray-100`}>
                  ...
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500">
                        Создано: {new Date(org.createdAt).toLocaleDateString()}
                      </p>
                    </div>
                  </div>
                  {org.isBlocked && (
                ...
                <div className="grid grid-cols-2 gap-2 text-sm text-gray-600 mb-3">
                  <div>
                    Пользователей: <span className="font-semibold">{org.userCount}</span>
                  </div>
                  <div>
                    Команд: <span className="font-semibold">{org.teamCount}</span>
                  </div>
                </div>
                <div className="flex justify-end pt-3 border-t border-gray-100">
                  <button
                    onClick={() => handleToggleOrganizationBlock(org)}
                    className={`${org.isBlocked ? "text-green-600" : "text-orange-500"} hover:opacity-80 text-sm font-medium`}>
            ...
            <table className="min-w-full leading-normal">
              ...
              <tbody>
                {organizations.map((org) => (
                  <tr
                    key={org.id}
                    className={org.isBlocked ? "bg-gray-50 opacity-75" : ""}>
                    ...
                    <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                      <span
                        className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${org.isBlocked ? "bg-red-100 text-red-800" : "bg-green-100 text-green-800"}`}>
