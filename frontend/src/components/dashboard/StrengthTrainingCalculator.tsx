...
export function StrengthTrainingCalculator({ exercises }: StrengthTrainingCalculatorProps) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-6 h-full flex flex-col relative">
      <h2 className="text-xl font-bold text-gray-900 mb-4">Калькулятор 5/3/1</h2>
        ...
        <table className="w-full text-sm text-left">
          ...
          <tbody>
            ...
            <tr className="bg-white border-b hover:bg-gray-50 align-top">
              <td className="px-4 py-3 font-medium text-gray-900 w-16">1</td>
                ...
                {loggingWeek === 1 ? (
                  <div className="flex flex-col items-end gap-2" onPointerDown={handleInputPointerDown} onKeyDown={handleInputKeyDown}>
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => setLoggingWeek(null)} className="text-xs text-gray-500 hover:text-gray-700">Отмена</button>
              ...
              <td className="px-4 py-3 text-right">
                {loggingWeek === 2 ? (
                  <div className="flex flex-col items-end gap-2" onPointerDown={handleInputPointerDown} onKeyDown={handleInputKeyDown}>
                      ...
                      />
                      <div className="flex gap-2">
                        <button onClick={() => setLoggingWeek(null)} className="text-xs text-gray-500 hover:text-gray-700" onPointerDown={handleInputPointerDown}>Отмена</button>
...
Work: 5x${calculateWeight(0.75)}кг, 3x${calculateWeight(0.85)}кг, 1+x${calculateWeight(0.95)}кг`
      ...
      {/* History Modal */}
      {showHistory && (
        <div className="absolute inset-0 z-50 bg-white/95 backdrop-blur-sm p-6 flex flex-col rounded-lg animate-in fade-in duration-200">
            <div className="flex justify-between items-center mb-6">
                <h3 className="text-lg font-bold text-gray-900">История тренировок</h3>
                <button 
            ...
            <div className="flex-1 overflow-auto">
             {loadingHistory ? (
                ...
                <div className="text-center py-8 text-gray-400">
                  {selectedExerciseId ? 'Нет записей для выбранного упражнения' : 'Выберите упражнение'}
                </div>
             )}
            </div>
        </div>
