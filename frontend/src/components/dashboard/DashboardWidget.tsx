...
export const DashboardWidget: React.FC<DashboardWidgetProps> = ({
  ...
  return (
    <Card
      className={`h-full flex flex-col relative transition-all duration-300 ${className} ${!isExpanded ? "overflow-hidden" : ""}`}
      noPadding={true}>
      {(title || headerActions) && (
        <div
          className={`flex justify-between items-center shrink-0 ${!isExpanded ? "px-4 h-full" : "p-4 border-b border-gray-100 dark:border-gray-700"}`}>
          {title && (
            <h2
              className={`font-bold text-gray-900 dark:text-white transition-all ${!isExpanded ? "text-base" : "text-xl"}`}>
              {title}
            </h2>
          )}
