...
export async function GET(
  ...
  try {
    ...
    // We using the standard WodsController which we made public for reading
    const backendApiUrl = new URL(`${backendUrl}/wods/${id}`)
    ...
    if (response.ok) {
      ...
      logApiError(`/wods/${id}`, new Error("Backend error"), {
        status: response.status,
        data,
      })
      return NextResponse.json(
        { message: data.message || "Failed to fetch wod" },
        { status: response.status },
      )
    }
  } catch (error) {
    logApiError(`/wods/${id}`, error)
