        ...
        <div className="flex items-center gap-6 p-4 bg-gray-50 rounded-2xl border border-gray-100">
          ...
          <div className="flex-1">
            ...
            <p className="text-blue-600 font-medium uppercase tracking-wider text-xs bg-blue-50 inline-block px-2 py-1 rounded-md mt-1">
              {user.role === 'TRAINER' ? 'Тренер' : user.role === 'ATHLETE' ? 'Атлет' : user.role}
            </p>
                ...
                >
                  {strengthResults.length > 0 ? (
                    strengthResults.map((result) => (
                      <div key={result.id} className="p-4 bg-white border border-gray-200 rounded-xl hover:border-blue-300 transition-colors shadow-sm">
                        <div className="flex justify-between items-start mb-2">
                          <h4 className="font-bold text-gray-900">{result.exercise?.name}</h4>
                          <span className="text-xs text-gray-400">{new Date(result.date).toLocaleDateString('ru-RU')}</span>
                        </div>
                        <div className="flex items-end gap-2">
                          <span className="text-2xl font-black text-blue-600">{result.weight}</span>
                ...
                >
                  {eventResults.length > 0 ? (
                    eventResults.map((result) => (
                      <div key={result.id} className="flex items-center justify-between p-4 bg-white border border-gray-200 rounded-xl hover:border-blue-300 transition-colors shadow-sm">
                        <div className="flex items-center gap-4">
                          <div className="w-12 h-12 bg-gray-50 rounded-lg flex flex-col items-center justify-center text-gray-500 border border-gray-100">
                             <span className="text-[10px] font-bold uppercase leading-none">{new Date(result.event?.eventDate || '').toLocaleDateString('ru-RU', { month: 'short' })}</span>
                             <span className="text-lg font-black leading-none">{new Date(result.event?.eventDate || '').getDate()}</span>
                          </div>
                          <div>
                            <h4 className="font-bold text-gray-900">{result.event?.title}</h4>
                            <p className="text-xs text-gray-400">{result.event?.exerciseType}</p>
                          </div>
                        </div>
                        <div className="text-right">
                          <p className="text-xl font-black text-blue-600">{result.time}</p>
                          <p className="text-[10px] text-gray-400 uppercase font-bold">Результат</p>
                        </div>
                      </div>
                    ))
                  ) : (
