...
export class OrganizationService {
  ...
  async getTrainerStats(trainerId: string) {
    ...
    const allMembers = ownedTeams.flatMap((team) =>
      team.members.map((m) => m.user),
    );
    const uniqueAthletes = Array.from(new Set(allMembers.map((u) => u.id))).map(
      (id) => allMembers.find((u) => u.id === id),
    );
    ...
    const athleteIds = uniqueAthletes.map((u) => u.id);

    // Получаем отдельные счетчики/ID для их объединения
    const recentStrengthResults =
      await this.prisma.strengthWorkoutResult.findMany({
        where: {
    ...
    const athleteNames = uniqueAthletes.map((u) =>
      `${u.name} ${u.lastName || ''}`.trim(),
    );
    const athleteNamesSimple = uniqueAthletes.map((u) => u.name); // Fallback if lastname missing in DB but present in Result

    const recentEventResults = await this.prisma.eventResult.findMany({
      where: {
        OR: [
          { userId: { in: athleteIds } },
    ...
    recentStrengthResults.forEach((r) => activeAthleteIds.add(r.userId));
    recentEventResults.forEach((r) => {
      if (r.userId) {
        activeAthleteIds.add(r.userId);
      } else {
        // Пытаемся сопоставить имя пользователя обратно с athleteId
        const athlete = uniqueAthletes.find(
          (u) =>
    ...
    const latestEventActivity = await this.prisma.eventResult.findMany({
      where: {
        OR: [
          { userId: { in: athleteIds } },
          { username: { in: [...athleteNames, ...athleteNamesSimple] } },
    ...
    const formattedStrengthActivity = latestStrengthActivity.map(
      (activity) => ({
        id: activity.id,
        athleteName:
    ...
    const formattedEventActivity = latestEventActivity.map((activity) => {
    ...
    const allActivity = [
      ...formattedStrengthActivity,
      ...formattedEventActivity,
    ]
