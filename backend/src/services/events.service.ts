...
export class EventsService {
  ...
  async updateEventStatus(eventId: string, status: 'COMPLETED' | 'FUTURE') {
  ...
  async deleteEvent(eventId: string, userId: string): Promise<void> {
    ...
    if (!hasPermission) {
      ...
      const requestorTeamIds = ownedTeams.map((t) => t.id);
      ...
      const allAdminTeamIds = [
        ...new Set([
          ...requestorTeamIds,
          ...adminMemberships.map((m) => m.teamId),
        ]),
      ];

      if (allAdminTeamIds.length > 0) {
        // Проверяем, является ли владелец события участником любой из этих команд
        const targetMembership = await this.prisma.teamMember.findFirst({
          where: {
            userId: event.userId,
    ...
    if (!hasPermission) {
      ...
      if (requestor) {
        if (requestor.role === 'SUPER_ADMIN') {
          ...
          requestor.role === 'ORGANIZATION_ADMIN' &&
          ...
          if (
            eventOwner &&
            eventOwner.organizationId === requestor.organizationId
          ) {
            hasPermission = true;
          }
        }
    ...
    if (!hasPermission) {
      throw new ForbiddenException(
        'You can only delete your own events or events in your team',
      );
    }
  ...
  async createEventResult(
    ...
    const eventResult = await this.prisma.eventResult.create({
      data: {
        time,
        username,
        eventId,
  ...
  async updateEventResult(resultId: string, info: string, actorId?: string) {
    ...
    if (result.notes) {
      if (Array.isArray(result.notes)) {
        ...
        try {
          const parsed = JSON.parse(result.notes) as string[] | string;
    ...
    const updatedResult = await this.prisma.eventResult.update({
      where: { id: resultId },
    ...
    if (!targetUserId && result.username) {
      const potentialUser = await this.prisma.user.findFirst({
        where: {
          OR: [
  ...
  async getEventResults(eventId: string) {
    ...
    try {
      return await this.prisma.eventResult.findMany({
  ...
  async toggleResultLike(resultId: string, userId: string) {
    ...
    const existingLike = await this.prisma.eventResultLike.findUnique({
      where: {
    ...
    if (existingLike) {
      // Unlike
      await this.prisma.eventResultLike.delete({
        where: { id: existingLike.id },
      });
      return { liked: false };
    } else {
      // Like
      await this.prisma.eventResultLike.create({
        data: {
          eventResultId: resultId,
          userId: userId,
        },
      ...
      if (!targetUserId && result.username) {
        const potentialUser = await this.prisma.user.findFirst({
          where: {
            OR: [{ name: result.username }, { email: result.username }],
          },
        });
        if (potentialUser) {
          targetUserId = potentialUser.id;
      ...
      if (targetUserId && targetUserId !== userId) {
        ...
        const name = liker
          ? `${liker.name} ${liker.lastName || ''}`.trim()
          : 'Someone';
