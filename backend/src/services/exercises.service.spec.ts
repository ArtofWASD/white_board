...
describe('ExercisesService', () => {
  ...
  describe('createExercise', () => {
    it('should create exercise without initial weight', async () => {
      prisma.userExercise.create.mockResolvedValue({
        id: 'ex1',
        name: 'Pushup',
      });
  ...
  describe('getExercisesByUserId', () => {
    it('should return exercises with calculated maxWeight', async () => {
      const mockExercises = [
        {
          id: 'ex1',
          records: [{ weight: 50 }, { weight: 60 }, { weight: 55 }],
        },
        {
          id: 'ex2',
          records: [], // No records
  ...
  describe('addExerciseRecord', () => {
    it('should add record to existing exercise', async () => {
      ...
      prisma.exerciseRecord.create.mockResolvedValue({
        id: 'rec1',
        weight: 70,
      });
    ...
    it('should throw NotFoundException if exercise does not exist', async () => {
      ...
      await expect(service.addExerciseRecord('ex1', 70)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
